...
class DialogAgent(AgentBase):
    ...
    def reply(self, x: Optional[Union[Msg, Sequence[Msg]]] = None, mem: Optional[Union[str, list[Msg], Msg]] = None) -> Msg:
        ...
        # if x['mem'] == 'add':#self.memory:
        self.memory.add(x)
        # elif x['mem'] == 'delete':
        #     self.memory.delete(x)
        if mem is not None:
        ...
        msg = Msg('system', response.text, role="system")
        # print(msg)
        # exit()
        # Record the message in memory
        if x['mem'] == 'add':  # self.memory:
            self.memory.add(msg)
            # print('self_mem add:',self.memory)
        elif x['mem'] == 'delete':
            self.memory.delete(0)
            # print('self_mem delete:', self.memory)
        # exit()
        # print('mem:',self.memory)
        # exit()
        return msg, self.memory
